...
Instead of drawing $y$ from $[0,1]$ we draw $x$ from $(-\infty, +\infty)$ using some proposal distribution. 
In the equation for the acceptance probability, we then replace each occurrence of $P_y(y)$ with 
$P_x(x) = P_y(y = \sigma(x)) \sigma'(x)$.
...
function metropolis_hastings(logprior, loglik, logjac, initial, randproposal, logproposal, n)
    ...
    for i in 1:n
        proposed = randproposal(current)
        current_ = logistic(current)    # current_ in in [0,1]
        proposed_ = logistic(proposed)  # proposed_ is in [0,1]
        # log p(current|data) = log p(current_ | data) + log partial(current_, current) 
...
see around Eqn 16.8 in Ben Lambert's A student's guide to Bayesian Statistics for a nice example
